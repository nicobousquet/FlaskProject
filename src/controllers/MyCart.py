...
def index():
    if 'email' in session:
        user_cart = CartModel.select_items_from_cart(session['email'])
        cart_total_price = get_cart_total_price(user_cart)
        return render_template('navbar.html') + render_template('mycart.html', user_cart=user_cart, cart_total_price=cart_total_price)
    else:
...
def add_to_cart(id):
    if 'email' in session:
        ...
        CartModel.insert_item_into_cart(session['email'], id, request.form['quantity'], request.form['radio_buttons'], price)
...
def remove_item(poster_id, quantity, size):
    CartModel.delete_item(session['email'], poster_id, quantity, size)
    return redirect(url_for('MyCart.index'))
