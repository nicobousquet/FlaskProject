...
def add_new_user():
    if not email_used(request.form['email']):
        UsersModel.insert_new_user(request.form['fname'], request.form['lname'], request.form['email'], request.form['password'])
...
def email_used(email):
    user = UsersModel.select_user_by_email(request.form['email'])
    if len(user) > 0:
        return True
    return False
